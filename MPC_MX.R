...
# Data is already in tabular form, balanced, almost pristine 
mx <- read.csv("Demand_and_Total_Income_Quarterly_1993_2021.csv", header = TRUE, sep = ",")
colnames(mx) <- c("PERIODO", "INGRESO", "CONSUMO")
# To change this, we call magrittr library and apply lubridate to PERIODO
mx %<>% 
  mutate(PERIODO = lubridate::dmy(PERIODO))
mx # take a look
...
# PERIOD variable and then value, 3 columns
df <- mx %>%
  select(PERIODO,CONSUMO,INGRESO) %>%
  gather(key = "variable", value = "value", -PERIODO)
head(df, 3)
...
P <- ggplot(df, aes(x = PERIODO, y = value)) +
  ...
  ggtitle("Evolución por Trimestre de la Demanda Comercial e Ingreso Disponible") +
  theme_minimal() +
  ylab("Valores en millones de MXN")
...
ingreso <-  ts(mx$INGRESO, start=c(1993,1), end=c(2021,4), frequency=4)
consumo <-  ts(mx$CONSUMO, start=c(1993,1), end=c(2021,4), frequency=4)
...
MPC = c/i
# MPC time series
MPCts <-  ts(MPC, start=1993, end=2021, frequency=4)

# take a look, there are spikes everywhere!!!
hchart(MPCts)

# what about the mean? do we expect something around 60% - 70% as the previous excercise?
# seems we are right mean value lingers around 71%
mean(MPCts)
...
#-- p-value > 0.05 no enough evidence to reject the Null hypothesis
#-- meaning that the time series of MPC is no-stationary.
pp.test(MPCts) #Voila! Stationarity thanks to many more observations!
# run Augmented Dickey-Fuller Test
# This drops a -2.5 value < 5% ergo, this is stationary
# Unit root is a characteristic of a time series that makes it non-stationary
# Technically speaking, a unit root is said to exist in a time series of the value of alpha = 1
# Null Hypothesis (H0): alpha=1 this is the case when P>5%
adf.test(MPCts) #Voila! again stationarity 
...
acf(MPCts, lag.max=20)
# Partial Autocorrelation Model
# Everything is within boundaries, Lag 1 seems to be significant
pacf(MPCts, lag.max=20)
...
# d= 0 is times the lagged indicators have been subtracted to make the data stationary.
# q = 0 number of forecast errors in the model
# ARIMA(0, 0, 0) - contains constant and white noise :)
# sigma^2 is 7.095 and ME is 0.0093 the model fits... OK, we guess.

tsdiag(modelo)
Box.test(residuals(modelo), type = "Ljung-Box")
#Fortunately P-value of 85% > 5% residuals are independent!!!!

#Forecast
pronostico <- forecast(modelo,h=11)
pronostico
plot(pronostico, ylab="consumer confidence", xlim=c(1993,2023))
#Looks neat
