...
pronostico
plot(pronostico, ylab="MPC", xlim=c(2010,2023))
...
predictor = ugarchforecast(modelo, n.ahead=4)
predictor
plot(predictor, ylab="consumer confidence", xlim=c(2004,2023), which=1)
...
# Data is already in tabular form, balanced, almost pristine 
# Download internal consumption index from INEGI BASE 2013
...
# We plot the previous dataframe to get a taste of how
# notice the steep downtrend during pandemic
...
#transform this tabla into series starting from 1993 (monthly series)
consumo <-  ts(mx$CONSUMO, start=c(1993,1,1), frequency=12)
#Decompose the signal to see the trend, noise and seasonal parts
ddata <- decompose(consumo, "multiplicative")
plot(ddata)

# take a look, beautiful signal for ARIMA!!
hchart(consumo)
...
#plots indicate seasonality, trend and many significant lags
